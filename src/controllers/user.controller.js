...
const loginUser=asyncHandler(async (req,res)=>{
    ...
    const isPasswordValid= await user.isPasswordCorrect(password)// don't use User
...
const logoutUser=asyncHandler(async(req,res)=>{
    await User.findByIdAndUpdate(
        ...
        {
            new:true// it returnn the updated user by we not use it so it is useless here but not break the logic
        }
...
const refreshAccessToken=asyncHandler(async(req,res)=>{
    const incomingResfreshToken=req.cookie.refreshToken|| req.body.refreshToken
    ...
    try {
        const decodedToken=jwt.verify(incomingResfreshToken,process.env.REFRESH_TOKEN_SECRET)
...
const getCurrentUser= asyncHandler(async (req,res)=>{
    return res
    .status(200)
    .json( new ApiResponse(200,{user:req.user},"current user fetched sucessfully"))
})

const updateAccountDetails = asyncHandler(async(req, res) => {
    const {fullName, email} = req.body

    if (!fullName || !email) {// wrong condition 
